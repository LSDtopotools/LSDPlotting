...
# This is for plotting a comparison of nuclide prodction
...
def get_concentration_data(Directory):
    ...
    FileName= Dirname+"Nuclide_Conc_vs_Erate.csv"
...
def plot_production_rates():
    ...
    # get the data
    prod_data = get_concentration_data(Directory)
    ...
    err_CRONUS = np.divide(np.subtract(N_CRONUS,N_Braucher),N_Braucher)
    err_newCRONUS = np.divide(np.subtract(N_newCRONUS,N_Braucher),N_Braucher)
    ...
    ax.plot(erate,N_CRONUS, "r-", label = "CRONUS",linewidth = 2)
    ax.plot(erate,N_Braucher, "k-", label = "Braucher",linewidth = 2)
    ax.plot(erate,N_newCRONUS, "b--", label = "new CRONUS",linewidth = 2)
    ...
    plt.xlabel('Denudation rate (g cm$^{-2}$ yr$^{-1}$)', fontsize = axis_size)
    plt.ylabel('$^{10}$Be Concentration (atoms g$^{-1}$)', fontsize = axis_size)
    ...
    plt.legend(handles, labels, numpoints = 3, loc=1, ncol=1, borderaxespad=0.5)  
    ...
    ax2.plot(erate,err_CRONUS, "r-", label = "CRONUS",linewidth = 2)
    ax2.plot(erate,err_newCRONUS, "b--", label = "newCRONUS",linewidth = 2)
    ...
    plt.xlabel('Denudation rate (g cm$^{-2}$ yr$^{-1}$)', fontsize = axis_size)
    plt.ylabel('Fraction difference in $^{10}$Be Conc', fontsize = axis_size)
    #plt.title('Cosmocalc / New_code',fontsize = label_size+6)
    ...
    plt.savefig(Dirname+"ConcentrationComparisonMethods.svg",format = Fileformat)     
    plt.show()
    plt.clf()
    ...
    # gendepth a grid. 
    gs2 = GridSpec(100,100,bottom=0.1,left=0.1,right=1.0,top=1.0) 
    ax3 = Fig2.add_subplot(gs2[10:95,10:90])    
    
    ax3.plot(erate,N_shielded, "k-", label = "Shielded 0.8",linewidth = 2)
    ax3.plot(erate,N_newCRONUS, "b--", label = "new CRONUS",linewidth = 2)
    ...
    plt.savefig(Dirname+"ConcentrationShielding.svg",format = Fileformat)      
    plt.show()
