...
def make_plots():
    ...
    # open file
    #DataDirectory =  "//s1143956.home.geos.ed.ac.uk/s1143956/DTMDataStore/data/FR/br_data/" 
    #DataDirectory = "K://data/FR/br_data/movernFiles/" 
    #DataDirectory = "C://basin_data//Italy//"
    DataDirectory = "C://basin_data//Babault//"    
    
    outfilename = "movern_sensitivity.movern"
    outfile_figname = DataDirectory+"Stats_movern"
    outfile_figformat = "pdf"
    outfilename = DataDirectory+outfilename
    of = open(outfilename,'w')


    for FileName in glob(DataDirectory+"*BFmovern_*.movern"): # assigns a number to each iteration (i.e. for every .tree file in the directory)
    #for FileName in glob(DataDirectory+"*BF_mover_n_*.tree"): # this was for CHILD runs that have slightly screwed up filenames
    ...
    plt.savefig(outfile_figname +'.' + outfile_figformat, format=outfile_figformat)
